...
- **パスフレーズ＝IDなので、重複は二重に弾く。** 起動時（`assertDistinctPassphrases`）で
  見抜けるのは「同じ文字列」と「平文とそのハッシュ」まで。**別々のソルトでハッシュ化された
  同一パスフレーズは原理的に検出できない**（それが scrypt の目的）ので、`authenticate` が
  複数一致で例外を投げる側も要る。黙って先頭を採ると、渡した相手が別人として振る舞い、
  本人も運用者も気づかない
...
#### 1Passwordが管理画面のパスフレーズを保存しない（2026-08-29、未解決）

**深追いしないこと。** 手動でアイテムを作れば入力補完は効き、実用上はそれで足りる。
以下は「同じ順番で潰し直さない」ための記録。

フォーム側の標準的な対応は入れ終わっている（`src/auth/admin-pages.ts`、`pages.ts`）:

- フォームに `autocomplete="off"` を付けない（補完では無視されるが、**保存の判定には効く**）
- `autocomplete="username"` の**編集可能な**入力欄を置いて対にする。
  `readonly` / `tabindex="-1"` にすると「入力できない欄」として候補から外れて効かない（実測）
- 同意画面は `type=password` が2つ（passphrase と sid）あってパスワード変更フォームに見えるので、
  SID と招待コードに `data-1p-ignore` / `data-lpignore` を付けて候補から外す

潰した仮説:

| 仮説 | 結果 |
|---|---|
| HTTP だから | ✗ `nginx.arigato-nas` は保存できている |
| `.arigato-nas` が公開サフィックスに無いから | ✗ 同上 |
| CSP `default-src 'none'` が拡張のiframeを塞いでいる | ✗ **アイコンは出る**。緩めた変更は根拠が消えたので戻した |
| 同意画面が別オリジン(claude.ai)へリダイレクトするから | 管理画面は同一オリジン完結なので、こちらには当てはまらない |

残っている有力な説は「**1Password が `arigato-nas` 全体を1サイトとして扱っていて、
既存の別エントリがあるため新規保存を提案しない**」。確かめるにはパスフレーズ欄の
アイコンを開いて、他の `*.arigato-nas` の資格情報が候補に出るか見る。
