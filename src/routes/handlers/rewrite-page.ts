...
export async function handleRewritePage(
  ...
  try {
    ...
    // フォークのデフォルトは scrapbox（upstream は markdown）。format 未指定で
    // Markdown 変換に落ちると、Cosense記法で書いた本文が黙って壊れる。
    ...
    // 他の書き込み系と同じく、書き込む前に記法をリントする。
    // ページ全体を置き換える操作なので、壊れた記法が入ったときの被害が一番大きい。
    const lintMode = getLintMode();
    const lintWarnings = lintMode === 'off' ? [] : lintScrapboxText(convertedBody);
    if (lintMode === 'strict' && lintWarnings.length > 0) {
      return formatError(formatLintWarnings(lintWarnings, false), errorDetails(), params.compact);
    }
      ...
      return {
        content: [{
          ...
          text: `rewrote: ${params.pageTitle} (${oldLineCount} -> ${newLineCount} lines)`
            + (lintWarnings.length > 0 ? ` | notation warnings: ${lintWarnings.length}` : '')
        }]
      };
    }
