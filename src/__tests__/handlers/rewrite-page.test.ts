...
describe('handleRewritePage', () => {
  ...
  describe('書き換え', () => {
    ...
    // フォークは format 既定が scrapbox（upstream は markdown）。
    // 既定で Markdown 変換に落ちると、Cosense記法で書いた本文が黙って壊れる。
    test('format 未指定時は変換せずそのまま書き込むこと', async () => {
      ...
      const patchFn = mockedPatch.mock.calls[0]![2] as (lines: unknown[]) => { text: string }[];
      expect(patchFn([]).map(line => line.text)).toEqual(['Test Page', '[* Cosense記法の見出し]']);
    });

    test("format='markdown' を明示したときだけ変換されること", async () => {
      const { convertMarkdownToScrapbox } = await import('@/utils/markdown-converter.js');
      const mockedConvert = convertMarkdownToScrapbox as jest.MockedFunction<typeof convertMarkdownToScrapbox>;
